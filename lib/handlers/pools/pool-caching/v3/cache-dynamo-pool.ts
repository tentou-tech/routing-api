...
import { log, metric, MetricLoggerUnit } from '@tentou-tech/smart-order-router'
...
export class DynamoCachingV3Pool extends DynamoCaching<string, number, Pool> {
  ...
  override async get(partitionKey: string, sortKey?: number): Promise<Pool | undefined> {
    if (sortKey) {
      ...
      if (cachedPoolBinary) {
        /* 'V3_DYNAMO_CACHING_POOL_HIT_IN_TABLE', 1, MetricLoggerUnit.None */
        ...
        /* 'V3_DYNAMO_CACHING_POOL_MISS_NOT_IN_TABLE', 1, MetricLoggerUnit.None */
        return undefined
      }
    } else {
      /* 'V3_DYNAMO_CACHING_POOL_MISS_NO_BLOCK_NUMBER', 1, MetricLoggerUnit.None */
      return undefined
    }
  }
